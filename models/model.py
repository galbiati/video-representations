...
class Model(object):
    ...
    # def build_half_enc(self, inputs, outputs, batchsize):
    #     inputs_ = tf.reshape(inputs, (-1, 60, 80, 3))
    #     outputs = tf.reshape(inputs, (-1, 60, 80, 3))
    #
    #     with tf.variable_scope('encoder'):
    #         encoded, encoder_layers = self.encoder(inputs_)
    ...
    def build_full(self, inputs, batchsize):
        ...
        inputs_ = tf.reshape(inputs, (-1, 60, 80, 3))
        with tf.variable_scope('encoder'):
            encoded, _ = self.encoder(inputs_)
            encoded = tf.reshape(encoded, (batchsize, -1, 1024))
